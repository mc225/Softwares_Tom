    ...
    methods (Access = protected)
        function img=acquireDirect(cam,nbFrames)
            ...
            for (triggerIdx=1:nbFrames)
                ...
                snapshot=double(snapshot(cam.regionOfInterest(1)+[1:cam.regionOfInterest(3)],cam.regionOfInterest(2)+[1:cam.regionOfInterest(4)],:));
                
                snapshot=0.2126*snapshot(:,:,1)+0.7152*snapshot(:,:,2)+0.0722*snapshot(:,:,3); %Objective luminance weighting
                snapshot=snapshot./(2^cam.bitsPerPixel-1);
                
                %Debug info
                if (max(snapshot(:))>=1)
                    logMessage('%u pixels are saturated!',sum(snapshot(:)==max(snapshot(:))));
                end
