classdef SLM < handle
    ...
    methods
        ...
        function slm=set.referenceDeflectionFrequency(slm,newReferenceDeflectionFrequency)
            if (any(abs(newReferenceDeflectionFrequency)>=1/2)),
                logMessage('The reference deflection is (%d,%d) [cycles/slm-pixel], while it should generally be smaller than 1/2th in absolute value',newReferenceDeflectionFrequency);
            end
    ...
    methods (Access = protected)
        function slm=executeModulatePreFunctor(slm,phaseValues,amplitudeValues)
            if (~isempty(slm.modulatePreFunctor))
                switch(nargin(slm.modulatePreFunctor))
                    case 0
                        if (nargout(slm.modulatePreFunctor)<=0)
                            slm.modulatePreFunctor();
                        else
                            slm=slm.modulatePreFunctor();
                        end
                    case 1
                        complexPupilFunction=amplitudeValues.*exp(1i*phaseValues);
                        if (nargout(slm.modulatePreFunctor)<=0)
                            slm.modulatePreFunctor(complexPupilFunction);
                        else
                            slm=slm.modulatePreFunctor(complexPupilFunction);
                        end
                end
            end
        end
        function slm=executeModulatePostFunctor(slm,phaseValues,amplitudeValues,imageOnSLM)
            if (~isempty(slm.modulatePostFunctor))
                switch(nargin(slm.modulatePostFunctor))
                    case 0
                        if (nargout(slm.modulatePostFunctor)<=0)
                            slm.modulatePostFunctor();
                        else
                            slm=slm.modulatePostFunctor();
                        end
                    case 1
                        complexPupilFunction=amplitudeValues.*exp(1i*phaseValues);
                        if (nargout(slm.modulatePostFunctor)<=0)
                            slm.modulatePostFunctor(complexPupilFunction);
                        else
                            slm=slm.modulatePostFunctor(complexPupilFunction);
                        end
                    case 2
                        complexPupilFunction=amplitudeValues.*exp(1i*phaseValues);
                        if (nargout(slm.modulatePostFunctor)<=0)
                            slm.modulatePostFunctor(complexPupilFunction,imageOnSLM);
                        else
                            slm=slm.modulatePostFunctor(complexPupilFunction,imageOnSLM);
                        end
                end
            end
        end
    ...
    methods (Static = true)
        function displaySize=detectResolution(displayNumber)
            ge = java.awt.GraphicsEnvironment.getLocalGraphicsEnvironment();
            screenDevices = ge.getScreenDevices();
            if ((displayNumber-round(displayNumber)<eps('single')) && displayNumber>0 && displayNumber<=length(screenDevices)),
